# from model import db, User1, UserSchema
...
def checkauthentication(useremail,password):
    ...
    flag=bcrypt.checkpw(password.encode('utf8'),pwd_db.encode('utf8'),)
    return flag
...
def upload_file(billId):
    ...
    # checking  if the  request has the file part
            ...
            if 'file' not in request.files:
                ...
                dir=id+"/"
                # os.mkdir(dir)
                target=os.path.join(app.config['UPLOAD_FOLDER'], dir)
                print(target)
